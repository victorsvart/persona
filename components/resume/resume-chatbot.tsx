...
export const ResumeChatbot: React.FC<ResumeChatbotProps> = ({
  ...
  const hasAutoGeneratedRef = useRef(false); // Track if we've already auto-generated
    ...
    try {
      const response = await fetch('/dashboard/resume/generate', {
        ...
        body: JSON.stringify({
          ...
          userMessage: 'Generate a professional resume tailored for this position',
          userExperience: {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to generate initial resume';
  ...
  useEffect(() => {
    ...
    const loadWelcomeMessage = async () => {
      try {
        ...
        const hasExistingResumes = application.resumes && application.resumes.length > 0;
        
        const welcomeTemplate = await ContentLoader.loadWelcomeMessage();
        let welcomeContent = ContentLoader.replacePlaceholders(welcomeTemplate, {
        ...
        if (hasExistingResumes && application.resumes) {
          welcomeContent += `\n\nI see you already have ${application.resumes.length} resume${application.resumes.length > 1 ? 's' : ''} for this application. You can ask me to generate a new version, make changes to the existing one, or refine specific sections. How can I help you today?`;
        }
        ...
        if (hasExistingResumes && application.resumes) {
          fallbackContent += `\n\nI see you already have ${application.resumes.length} resume${application.resumes.length > 1 ? 's' : ''} for this application. You can ask me to generate a new version or make changes to the existing one.`;
        }
